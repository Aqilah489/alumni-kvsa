...
// Get user info
$stmt = $connect->prepare("SELECT nama, emel FROM ketua_program WHERE kp_id = ?");
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $connect->prepare("SELECT password FROM ketua_program WHERE kp_id = ?");
    $stmt->execute([$_SESSION['user_id']]);
    $current = $stmt->fetch();
    ...
    if(!password_verify($password_lama, $current['password'])) {
        ...
        $stmt = $connect->prepare("UPDATE ketua_program SET password = ? WHERE kp_id = ?");
        ...
        <div class="form-body">
            ...
            <div class="info-user">
                ...
                <div class="info-user-badge">
                    <span class="badge-role">Ketua Program</span>
                </div>
            </div>
